  ...
  const handleSort = (value: string) => {
    ...
    const sorted = [...carts].sort((a, b) => {
      switch (value) {
        ...
        case "customerName":
          return (a.user?.first_name || "").localeCompare(
            b.user?.first_name || "",
          );
        case "totalValue":
        ...
        <Select onValueChange={handleSort} defaultValue={sortBy}>
          ...
          <SelectContent>
            <SelectItem value="updated_at">
              {t("carts.sortOptions.lastUpdated")}
            </SelectItem>
            <SelectItem value="customerName">
              {t("carts.sortOptions.customerName")}
            </SelectItem>
            <SelectItem value="totalValue">
              {t("carts.sortOptions.cartValue")}
            </SelectItem>
          </SelectContent>
        </Select>
      </div>
              ...
              <TableCell>
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    <DropdownMenuLabel>
                      {t("carts.actions.title")}
                    </DropdownMenuLabel>
                    <DropdownMenuItem
                      onClick={() => handleAction("sendEmail", cart.id)}
                    >
                      {t("carts.actions.sendEmail")}
                    </DropdownMenuItem>
                    <DropdownMenuItem
                      onClick={() => handleAction("applyDiscount", cart.id)}
                    >
                      {t("carts.actions.applyDiscount")}
                    </DropdownMenuItem>
                    <DropdownMenuItem
                      onClick={() => handleAction("recoverCart", cart.id)}
                    >
                      {t("carts.actions.recoverCart")}
                    </DropdownMenuItem>
                    <DropdownMenuSeparator />
                    <DropdownMenuItem
                      onClick={() => handleAction("deleteCart", cart.id)}
                    >
